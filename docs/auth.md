...
- `POST /auth/login` – email/mot de passe → retourne tokens (access + refresh)
- `POST /auth/login/social` – accepte un token OAuth (Google, Apple, etc.)
- `POST /auth/login/magic-link` – demande un lien magique par email
- `GET /auth/verify-magic-link?token=...` – échange un magic link contre des tokens

## 3. **Endpoints de tokens**
...
## 7. **Endpoints de session / appareils**
...
## 8. **Endpoints de gestion de compte**
...
- `POST /auth/me/email` – changer d'email (nécessite validation)
...
## 10. **Endpoints administratifs**

- `POST /auth/admin/impersonate/{user_id}` – se faire passer pour un autre utilisateur
- `DELETE /auth/admin/users/{user_id}/sessions` – révoquer toutes les sessions d'un utilisateur
- `POST /auth/admin/users/{user_id}/ban` – bannir un compte
- `PATCH /auth/admin/users/{user_id}/roles` – modifier les rôles
...
X-Refresh-Token: <refresh_token>  (optionnel)
```
